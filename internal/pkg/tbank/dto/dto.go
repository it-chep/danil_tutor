...
func (r *InitResponse) ToQr(ctx context.Context, adminID int64, cred Credentials) *http.Request {
	tPayment, _ := strconv.Atoi(r.PaymentID)
	getQr := &GetQrRequest{
		TerminalKey: cred.UsersConf[adminID].TerminalKey,
		PaymentId:   int64(tPayment),
	}
	getQr.GenerateToken(cred.UsersConf[adminID].Password)
